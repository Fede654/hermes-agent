...
def run_conversation(
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        # Upstream folded the bare ``*_attempted`` flags into ``TurnRetryState``
        # (``_retry`` above). ``kimi_auth_retry_attempted`` is Nico-fork-only
        # (Kimi OAuth retry) and has no field there, so it stays a loose local.
